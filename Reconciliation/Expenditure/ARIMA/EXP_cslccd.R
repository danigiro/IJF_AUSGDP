...
for (j in 1:test_length) { #test_length
  ...
  # bCCCexod (basef + CCC + exogenous + diagonal cov) ----
  Start <- Sys.time()
  objb <- suppressWarnings(lccrec(basef = basef, C = C, nl = nl, 
                                  CCC = TRUE, weights = fixv[-c(1:NROW(C))]))
  End <- Sys.time()
  ...
  # bCCCendod (basef + CCC + endogenous + diagonal cov) ----
  Start <- Sys.time()
  objbe <- suppressWarnings(lccrec(basef = basef, C = C, nl = nl, const = "endo",
                                   CCC = TRUE, weights = fixv))
  End <- Sys.time()
...
save(DFcslev, time_cslev, 
     file = "./Reconciliation/Expenditure/ARIMA/EXP_cslccd.RData")
