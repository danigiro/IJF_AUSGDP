...
print("------- Mean -------")
try(expr = {
  load("./BaseForecasts/Income/INC_means_sa.RData")
  DFmean <- mvdf
  DFmean <- DFmean %>% rename(Forecasts = Mean) %>%
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_csmix.RData")
  DFmix <- DFmix %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_cslccd_sa.RData")
  DFcslev <- DFcslev %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_CCCmix.RData")
  DFmix <- DFmix %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_CCCmix_red.RData")
  DFmix <- DFmix %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_cslccd_mLCC.RData")
  DFcslev <- DFcslev %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
...
print("--------------------------")
try(expr = {
  load("./Reconciliation/Income/ARIMA/INC_cslccd_bLCC.RData")
  DFcslev <- DFcslev %>% 
    add_column(K = 1) %>% 
    mutate(Series = factor(Series, series, ordered = TRUE))
  i <- i + 1
